// Samp Journal


const SampleJournalSections =  [


  // Begin journal_1 ------------------------- >>
 
    {
      id: 'jsection_1',
      title: 'First thoughts about Chit Git',
      
      content: " <div>So I read that there are lot’s of <strong style=\"color: rgb(107, 36, 178);\">benefits</strong><span style=\"color: rgb(107, 36, 178);\"> </span>to keeping a journal … things like - it helps</div><ul><li><span style=\"color: rgb(107, 36, 178);\">achieve goals,</span></li><li><span style=\"color: rgb(107, 36, 178);\">inspiration, </span></li><li><span style=\"color: rgb(107, 36, 178);\">more self-confidence, </span></li><li><span style=\"color: rgb(107, 36, 178);\">improve memory, more creativity</span></li></ul><div>… blah blah blah.&nbsp;</div><div>&nbsp;</div><div>For me, the reason that resonated is that maybe it can <u style=\"color: rgb(107, 36, 178);\">reduce my stress</u>.</div><div>&nbsp;</div><div>I do like the idea of keeping track of things ...so here goes.</div>",

      journalDate: "2021-01-03T16:21:33.000Z",
      lastEdit: "2021-01-03T16:21:33.000Z",

      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',

      peopleArray: [],
      keywordArray: [],


    },
    {
      id: 'jsection_2',
      title: 'New Year',
      
      content: "<div>This year I’d like to get more balance in my life … spend more time with Mary.  I'd really like to take Mary on that vacation I promised her, but had to postpone twice last year because of work.</div><div>&nbsp;</div><div>Although the job is good, and I like my boss and most of my coworkers -- I really would like to get out of the medical industry… maybe something in sports.&nbsp;</div><div>&nbsp;</div><div>When I started my career I thought medical would be great because it helps people and makes money… but teh realit is - it's all about the money... <strong><u>only money </u></strong> &nbsp;Helping people is secondary or worse still - not even on the agenda.&nbsp;My last company was&nbsp;like that - the product actually didn’t work - but they forged ahead anyway.</div><div>&nbsp;</div><div>Hope to move into the new house by summer.&nbsp;When we bought it 6 months ago it looked bad, but I thought it would only take a few months to fix it up.&nbsp;Looks like it’ll still take another 6 months before it’s livable.&nbsp;But the savings gains by doing the work myself and along with the value increases from the improvements we’re making make it worthwhile.&nbsp;PS.&nbsp;Mary is taking on landscaping herself - she’s amazing.</div><div>&nbsp;</div><div>Also this year, one of my goals will be to put some <strong style=\"color: rgb(230, 0, 0);\">savings in the bank … at least $10k</strong>.&nbsp;Tired of living without a cushion.</div>",

      
      journalDate: "2021-01-03T16:21:33.000Z",
      lastEdit:"2021-01-03T16:21:33.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',

      peopleArray: ['Mary B'],
      keywordArray: ['goals'],
 

    },

    {
      id: 'jsection_3',
      title: '',
      
      content: "<div>Tomorrow my new project starts.&nbsp;The last project I headed did great, so I get to head the team for the next big thing.</div><div>&nbsp;</div><div>Still didn’t get to pick my own team, although I really enjoyed working with Mark (engineering) on the last project.</div><div>&nbsp;</div><div>Marketing chose Cynthia for the team … we really don’t like each other.&nbsp;But then again, don’t think it’s personal…I don’t think Cynthia likes anyone.</div><div>Tomorrow it begins.</div>",

      journalDate: "2021-01-03T16:26:33.000Z",
      lastEdit: "2021-01-03T16:26:33.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',

      peopleArray: ['Cyn Terhor', 'Mark D'],
      keywordArray: ['project team', 'work'],
      

    },

    {
      id: 'jsection_4',
      title: 'Time to get real',
      
      content: "<div>Think I’ll make the vacation thing a surprise for Mary… and happen in May…&nbsp;That way we can celebrate Mary’s birthday somewhere great … So that makes a May 1 target.<span style=\"color: rgb(107, 36, 178);\">&nbsp;</span></div><div><br></div><div><strong style=\"color: rgb(107, 36, 178);\"><span class=\"ql-cursor\">\ufeff</span>Note to self</strong><span style=\"color: rgb(107, 36, 178);\">&nbsp;</span>-- need to get Mary’s boss in on the surprise up front so he can block out the time for her… like this month.</div><div>&nbsp;</div><div>Gonna try to lose 20 pounds. It’d be great if I could lose it by the time we go on vacation. Think I'll begin tomorrow. &nbsp;</div><div> Diet, exercise  - Ugg &nbsp;</div>",

      journalDate: "2021-01-03T16:31:33.000Z",
      lastEdit: "2021-01-03T16:31:33.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',

      peopleArray: ['Mary B'],
      keywordArray: ['diet' ,'exercise', 'vacation'],
  

    },


    {
      id: 'jsection_5',
      title: 'Deck',
      
      content: "I think the next task I’ll tackle will be the deck for the house.  When spring comes, it’ll be nice to have a place to sit outside, have a beer and zen out.",

      journalDate: "2021-01-03T18:10:00.000Z",
      lastEdit: "2021-01-03T18:10:00.000Z",

      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',

      peopleArray: [],
      keywordArray: ['deck'],
      

    },


  // Begin journal Jan 6 ------------------------- >>
 
  

    // Begin journal Jan 7 ------------------------- >>
 
    {
      id: 'jsection_7',
      title: '',
      
      content: "<div>  Started meetings with the individual members.&nbsp;Mark was great - lot’s of ideas already.&nbsp;Cyn - not so much.&nbsp;It’s obvious she is not pleased that I got the manager gig and not her.&nbsp;She’s really not great at hiding what she’s thinking… but then again - neither am I.</div><div>  </div>",
  
      journalDate: "2021-01-06T16:21:55.000Z",
      lastEdit: "2021-01-06T16:21:55.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Cyn', 'Mark D'],
      keywordArray: ['project team'],
     
  
    },

    // Begin journal Jan 9 ------------------------- >>

    {
      id: 'jsection_8',
      title: '',
      
      content: "<div>Found a great video on you tube on how to build a deck.&nbsp;The design is not what I want - but the rest of it works well.</div><div><a href=\"https://www.youtube.com/watch?v=xMGgQAYOiM0\" rel=\"noopener noreferrer\" target=\"_blank\">https://www.youtube.com/watch?v=xMGgQAYOiM0</a></div><div>&nbsp;</div><div>Told Jerry about the deck… he offered to help and his little brother’s.&nbsp;I was going to ask Billy’s help anyway because he’s the builder in the family.&nbsp;Jerry is more entertainer than worker… :-)</div><div>&nbsp;</div><div>Regarding the vacation - my boss let me block out the first week in May.&nbsp;Would have liked to make it 2 weeks, but just don’t have that much time.&nbsp;Now I have to talk to Mary’s boss.</div>",
  
      journalDate: "2021-01-09T16:21:33.000Z",
      lastEdit: "2021-01-09T16:21:33.000Z",
      chitLink: 'personalChit',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Mary B', 'Jerry G', 'Billy'],
      keywordArray: ['deck', 'vacation'],
      
  
    },

    {
      id: 'jsection_9',
      title: 'First project team meetings',
      
      content: "<div> Started meetings with the individual members last week.&nbsp;<strong>Mark </strong>was great - lot’s of ideas already.&nbsp;<strong>Cyn</strong> - not so much.&nbsp;It’s obvious she is not pleased that I got the project manager gig and not her.&nbsp;She’s really not great at hiding what she’s thinking… but then again - neither am I.&nbsp;That’s why we’re oil and water.</div><div> </div>",
  
      journalDate: "2021-01-09T16:21:33.000Z",
      lastEdit: "2021-01-09T16:21:33.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Cyn Ter', 'Mark D'],
      keywordArray: ['project team' , 'work'  ],
      
  
    },

    
    {
      id: 'jsection_10',
      title: '',
      
      content: "<div>Exercised every day my first week of diet and exercise… but blew it big time on Friday with the diet.&nbsp;The good news is I was back on diet yesterday… and today - so far so good.</div>",
  
      journalDate: "2021-01-09T16:21:35.000Z",
      lastEdit: "2021-01-09T16:21:35.000Z",
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['diet', 'exercise'  ],
      
  
    },

    // Begin journal Jan 14 ------------------------- >>

    {
      id: 'jsection_11',
      title: 'Shelly = Karen',
      
      content:  '<div>She did it again -- I have my own personal “Karen”&nbsp;living above me.&nbsp;Dog hair and butts… and this time candy wrappers.<span style="color: rgb(153, 51, 255);">&nbsp;What an inconsiderate slob</span>.&nbsp;Need to create a log.</div>',
  
      journalDate: '2021-01-14T06:08:53.000Z',
      lastEdit:'2021-01-14T06:08:53.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Shelly-neighbor'],
      keywordArray: ['neighbor dispute' ],
      
  
    },

    {
      id: 'jsection_12',
      title: 'Behind schedule',
      
      content: "<div>Our project is already behind schedule … but never had a project plan proceed as planned.&nbsp;Don’t think it’s too big a deal right now … lose another week and then I’ll begin to stress.</div>",
  
      journalDate: '2021-01-14T06:08:54.000Z',
      lastEdit: '2021-01-14T06:08:54.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['project team', 'work'   ],
      
  
    },


    {
      id: 'jsection_13',
      title: '',
      
      content: "<div>Diet and exercise are proceeding… getting into the groove with the exercise - that’s because I’m just getting up and getting it over with before I’ve had a chance to think about it.</div>",
  
      journalDate: '2021-01-14T06:08:55.000Z',
      lastEdit: '2021-01-14T06:08:55.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['diet', 'exercise' ],
      
  
    },


    {
      id: 'jsection_14',
      title: 'Deck plans',
      
      content: '<div>Created plans for the deck… showed them to Mary. &nbsp;Gave me thumbs up … Know who the boss is around here. </div><div>&nbsp;</div><div>It’s a lot larger than&nbsp;originally planned… but now it wraps around the house and let’s the master bedroom has access to it.&nbsp;Of course will need to break down a wall to create a door… but will be nice with sliding glass doors --- AND a hot tub.&nbsp;</div><div>&nbsp;</div><div>Have to contract this one out&nbsp;- another<span style="color: rgb(230, 0, 0);"> $3k to $5k</span>… but it’ll really make the outside of the house AND - really brighten up the master.</div><div>&nbsp;</div><div>The deck will be a little too high for the hot tub to sit on the ground -- maybe prop it up or reinforce the deck to hold the weight.</div><div>&nbsp;</div><div>&nbsp;I’ll get together with <strong>Jerry </strong>and <strong>Billy </strong>this weekend.&nbsp;See what they have to say about the plans… and what we need to buy.&nbsp;</div>',
  
      journalDate: '2021-01-14T06:08:56.000Z',
      lastEdit: '2021-01-14T06:08:56.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Mary B','Billy', 'Jerry G'],
      keywordArray: ['deck'  ],

     },

    // Begin journal Jan 24 ------------------------- >>

    {
      id: 'jsection_15',
      title: 'Wakeboard Boots',
      
      content: '<div>Been a while since I’ve written here… But last night I woke up with a great idea.&nbsp;<strong style="color: rgb(161, 0, 0);" class="ql-size-large">Rotatable wakeboard</strong><strong>…</strong></div><div>&nbsp;</div><div>Slipped a disc in my lower back Friday .. reminded me of what happened wakeboarding last summer.&nbsp;I’m ok on the back and forths crossing the wake and jumping - but the constant, persistent twist following the boat when not crossing the wake really strains my lower back.</div><div>&nbsp;</div><div>The idea is to have a mechanical boot assembly that allows me to face forward - like on 2 ski’s … that slides and locks into the perpendicular&nbsp;for jumping and tricks.</div><div>&nbsp;</div><div>I imagine it to be triggered with some sort of mechanical toe switch inside the boot.&nbsp;</div><div>&nbsp;</div><div>Going to think on it for a while -- could be a great product.. and maybe even a business.&nbsp;Wouldn’t that be great.</div>',
  
      journalDate:'2021-01-24T23:51:38.000Z',
      lastEdit:'2021-01-24T23:51:38.000Z',
      chitLink: '  ',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: [ 'wakeboard'],
      
  
    },


    {
      id: 'jsection_16',
      title: 'Mary boss on board',
      
      content: '<div>Talked to Mary’s boss about the surprise vacation in May.&nbsp;He’s all in on working the secret.&nbsp;He’ll block out 10 days for her … but not let her know.&nbsp;</div><div>&nbsp;</div><div>I’ve been thinking Fiji -- Mary would love that.&nbsp;But with the distance, need to use a few extra vacation days. </div><div>&nbsp;</div><div>The problem is that it can’t be a complete surprise because I would need to let her know earlier so she can <strong class="ql-size-large" style="color: rgb(230, 0, 0);">get a passport</strong>.&nbsp;Don’t think she has one. </div>',
  
      journalDate: '2021-01-24T23:52:38.000Z',
      lastEdit:'2021-01-24T23:52:38.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Mary B'],
      keywordArray: [ 'vacation' ],
      
  
    },

    
    {
      id: 'jsection_17',
      title: '',
      
      content: '<div>On the weight front - lost 5 pounds so far.&nbsp;It’s hard not to weigh myself every day … but read somewhere it’s better to just do the weigh-in’s once a week.</div>',
  
      journalDate: '2021-01-24T23:53:38.000Z',
      lastEdit: '2021-01-24T23:53:38.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['diet'],
      
  
    },

    
    {
      id: 'jsection_18',
      title: 'Wakeboard club',
      
      content: '<div> Jerry set me up last night at the ski club meeting.&nbsp;He nominated me for Treasurer … and before I knew it, I’ve got another group of responsibilities added to my plate. </div><div>&nbsp;</div><div>But I got even - he’s now the Secretary … so he’s got to organize all the trips. </div><div> </div>',
  
      journalDate: '2021-01-24T23:54:38.000Z',
      lastEdit: '2021-01-24T23:54:38.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['wakeboard'],
      
  
    },


    //   --- Jan 27 --------------------

    {
      id: 'jsection_19',
      title: 'Trouble',
      
      content: '<div><strong style="color: rgb(230, 0, 0);">Cynthia </strong>is a real work … First she missed another meeting … then she does a half assed job on her only assignment which was 2 weeks late.</div><div>&nbsp;</div><div>Saw no recourse but to cc her boss… which really set her off.&nbsp;She came into my office all red faced -- not loud but yelling all the same -- called me all sorts of names.</div><div>&nbsp;</div><div>Never got a word in - clearly this isn’t going to work out.&nbsp;Need to figure out how to get her work done without her.</div>',
  
      journalDate: '2021-01-27T23:58:38.000Z',
      lastEdit: '2021-01-27T23:58:38.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Cyn Ter'],
      keywordArray: ['project team'],
      
  
    },

      
  
    {
      id: 'jsection_20',
      title: 'Vacation booked',
      
      content: '<div>Booked a 5 star hotel in Fiji… it’s on a small atoll but got a great deal on an ocean view suite.&nbsp;The flights were more expensive than I wanted… but the package deal for the hotel room made it worthwhile. 10 hours in coach is going to suck though.&nbsp;But it was nicer hotel and room for 10 days and coach flights… or …&nbsp;2nd tier hotel for 10 days and business class.&nbsp;Easy choice.</div>',
  
      journalDate: '2021-01-27T23:58:39.000Z',
      lastEdit: '2021-01-27T23:58:39.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['vacation'],
      
  
    },

    {
      id: 'jsection_21',
      title: 'Deck hot tub',
      
      content: '<div>Going to start on the deck this weekend - weather permitting.&nbsp;<strong>Billy </strong>redid the designs to accommodate the hot tub.&nbsp;It’s going to be inset with padding around the outside.&nbsp;Nice touch.&nbsp;Lattice underneath the deck has been added.&nbsp;Afraid it’s gonna run over budget.&nbsp;But it’ll be used - a lot.</div>',
  
      journalDate: '2021-01-27T23:59:39.000Z',
      lastEdit:  '2021-01-27T23:59:39.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Billy'],
      keywordArray: ['deck'],
      
  
    },



    {
      id: 'jsection_22',
      title: 'Progress',
      
      content: " <div>Been a while since I've written.</div><div><br></div><div>Lost 10 pounds since I started my exercise and diet program. I like the exercise now... the diet not so much</div>",
  
      journalDate: '2021-02-16T18:03:40.000Z',
      lastEdit:  '2021-02-16T18:03:40.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: [],
      keywordArray: ['diet'],
      
  
    },

    {
      id: 'jsection_23',
      title: 'Deck',
      
      content: "<div>Finished the deck with the help of Billy and Jerry and Mark even helped - it’s beautiful.&nbsp;Put hot tub on order yesterday.</div>",
  
      journalDate: '2021-03-14T17:03:40.000Z',
      lastEdit:  '2021-03-14T17:03:40.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Billy', 'Jerry'],
      keywordArray: ['deck'],
      
  
    },

    {
      id: 'jsection_24',
      title: 'More Deck',
      
      content: '<div>Deck painted, sealed with hot tub installed.&nbsp;Can’t say it was the most fun I’ve had - but really close.&nbsp;Couple that with the satisfaction of accomplishment and it is definitely something to celebrate.&nbsp;&nbsp;<span class="ql-size-large" style="color: rgb(107, 36, 178);">Mary loves it.</span></div>',
  
      journalDate: '2021-03-22T17:03:40.000Z',
      lastEdit:  '2021-03-22T17:03:40.000Z',
      chitLink: '',
                  
      mirrorType: '',
      mirrorId: '',
  
      peopleArray: ['Billy', 'Jerry'],
      keywordArray: ['deck'],
      
  
    },














    // {
    //   id: 'ajsection_66',
    //   title: 'Test 1',
      
    //   content: '<div>Test dec 27</div>',
  
    //   journalDate: '2020-12-27T23:59:39.000Z',
    //   lastEdit:  '2020-12-27T23:59:39.000Z',
    //   chitLink: '',
                  
    //   mirrorType: '',
    //   mirrorId: '',
  
    //   peopleArray: ['Billy'],
    //   keywordArray: ['deck'],
      
  
    // },

    
    // {
    //   id: 'bjsection_66',
    //   title: 'Test 2',
      
    //   content: '<div>Test nov 27</div>',
  
    //   journalDate: '2020-11-27T23:59:39.000Z',
    //   lastEdit:  '2020-11-27T23:59:39.000Z',
    //   chitLink: '',
                  
    //   mirrorType: '',
    //   mirrorId: '',
  
    //   peopleArray: ['Billy'],
    //   keywordArray: ['deck'],
      
  
    // },







]// end SampleJournal

export default SampleJournalSections